...
//  Web-based authentication and session coordinator
//

import SwiftUI
import AuthenticationServices

/// Session info returned from authentication.
...
/// Coordinates the active session and web-based authentication.
@Observable
class WebAuthService: NSObject {
    ...
    var authToken: String? { currentSession?.token }
    var currentUser: User? { currentSession?.user }
    ...
    func setSession(_ session: WebSession) {
        ...
        AuthRepository.shared.saveSession(session)
    }

    func clearSession() {
        currentSession = nil
        AuthRepository.shared.deleteSession()
    ...
    func switchToSession(_ session: WebSession) {
        setSession(session)
        if let gatewayURL = APIService.shared.gatewayURL.emptyToNil {
    ...
    private func loadSession() {
        // Prefer Keychain; fall back to UserDefaults and migrate.
        if let session = AuthSessionStore.loadSession() {
            currentSession = session.isExpired ? nil : session
        } else if let session = AuthSessionStore.migrateFromUserDefaultsIfNeeded() {
            currentSession = session.isExpired ? nil : session
        }
    ...
    func migrateFromLegacyIfNeeded() async {
        ...
        do {
            ...
            await MainActor.run { self.setSession(session) }
    ...
    // MARK: - Web Authentication (legacy / fallback)
...
enum WebAuthError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .noCallback: return "No response from authentication server"
        case .invalidCallback: return "Invalid response from authentication server"
        case .missingToken: return "Authentication token not received"
        case .serverError(let message): return "Server error: \(message)"
        case .discoveryFailed: return "Could not connect to Fluxer. Please try again."
